...
namespace EzySlice {
	...
	public sealed class Slicer {
		 ...
		 */
        public static _Sliced Slice(GameObject obj, Plane pl, TextureRegion crossRegion, Material crossMaterial) {
        ...
        public static EzySlice.Plane GetRandomPlane()
        {
        ...
        public static _Sliced Slice(Mesh sharedMesh, Plane pl, TextureRegion region, int crossIndex) {
			if (sharedMesh == null) {
				return null;
			}
            ...
            Slice(sharedMesh, GetRandomPlane(), region, crossIndex);
		 ...
		 */
		private static _Sliced CreateFrom(SlicedSubmesh[] meshes, List<Triangle> cross, int crossSectionIndex) {
			...
			return new _Sliced(upperHull, lowerHull);
		}
